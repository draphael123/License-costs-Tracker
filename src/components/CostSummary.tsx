  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500 mt-1">Annual licensing costs breakdown</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 bg-gray-50 border-b border-gray-200">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          ...
          <div className="text-2xl font-bold text-gray-900 mt-1">{providerCosts.length}</div>
          <div className="text-xs text-gray-400 mt-1">{mdCount} MD, {npCount} NP</div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="text-sm text-gray-500">Avg Cost per Provider</div>
          <div className="text-2xl font-bold text-gray-900 mt-1">
            ${Math.round(totalAnnualCost / providerCosts.length).toLocaleString()}
          </div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="text-sm text-gray-500">DEA Annual (per provider)</div>
          <div className="text-2xl font-bold text-gray-900 mt-1">
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">States</th>
              <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Annual Cost</th>
            </tr>
          </thead>
              ...
              <tr key={provider.provider} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-medium rounded ${
                    provider.type === 'MD' ? 'bg-purple-100 text-purple-700' : 'bg-teal-100 text-teal-700'
                  }`}>
                    {provider.type}
                  </span>
                </td>
                    ...
                    {provider.services.map(service => (
                      <span
                        ...
                        className={`px-1.5 py-0.5 text-xs rounded ${
                          service === 'TRT' ? 'bg-cyan-100 text-cyan-700' :
                          service === 'HRT' ? 'bg-pink-100 text-pink-700' :
                          service === 'GLP' ? 'bg-purple-100 text-purple-700' :
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right">
                  <div className="font-semibold text-gray-900">${provider.totalAnnualCost.toLocaleString()}</div>
                  <div className="text-xs text-gray-500">incl. DEA</div>
                </td>
              </tr>
            ))}
          </tbody>
